    ...
    def modeling_rf(self) :
        ...
        * 출력
        ...
        tuner = CustomSklearnTuner(
                ...
                cv=model_selection.StratifiedKFold(5, shuffle = True, random_state = 1234), # KFold 외에 Train/Test Split 사용 시 -> model_selection.PredefinedSplit (Train Set을 -1로, Test Set을 1로 index list 생성)
                directory='.',
...
class MODELING_CNN():
    ...
    def _fit(self):
        ...
        If False, the model weights obtained at the last step of training are used
        """
        self.EarlyStopping = tf.keras.callbacks.EarlyStopping(monitor = 'val_loss', mode = 'min', patience = 20,
                                                         restore_best_weights=True, verbose = 1) 
...
class cnn_model(HyperModel):
    ...
    def build(self, hp):
        ...
        sPool = hp.Choice("Pooling", ["Max","Avg"])
