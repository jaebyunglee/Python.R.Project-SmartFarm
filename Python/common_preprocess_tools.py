...
class common_preprocess():
    ...
    '''
    - (공통) 이상치/결측치 처리 및 시간대별 요약 통계량 생성
    '''
    def preprocess_fn(self) -> pd.DataFrame :
        ...
        * 출력
        ...
        col_list = [s for s in self.final_summary_df.columns if '시간' not in s]
        grp_window = int(np.ceil(np.sqrt(len(col_list))))
        fig, ax = plt.subplots(grp_window,grp_window,figsize = (42,21))
        ax = ax.flatten()
        org_final_summary_df = self.final_summary_df.copy()
        
        for idx,col in enumerate(col_list) :
            na_pos = list(np.where(self.final_summary_df[col].isna())[0])
            if len(na_pos) > 0 :
                print(f">> {col} - 선형 보간법 적용")
                # 선형보간법 적용
                self.final_summary_df[col] = self.final_summary_df[col].interpolate(method='spline', order = 2)

                # 선형보간법 적용 후 데이터 그래프
                x = self.final_summary_df['시간']
                y = self.final_summary_df[col]
                ax[idx].plot(self.final_summary_df['시간'],self.final_summary_df[col], label = 'Inter', color = 'red')
#                 ax[idx].set_facecolor("lightgray")

            # 오리지널 데이터 그래프
            x = org_final_summary_df['시간']
            y = org_final_summary_df[col]
            ax[idx].plot(x,y, label = 'Original', color = 'black')  
            ax[idx].legend(loc = 'upper right')

        # 나머지 그래프 없애기  
        [ax[s].axis("off") for s in range(len(col_list),(grp_window**2))]       
...
#         self.final_summary_df.iloc[:,1:] = self.final_summary_df.iloc[:,1:].interpolate(method='linear')
    ...
    '''
    - (학습) 종속변수/ 설명변수 지정 및 시차 변경된 설명변수 생성
    '''
    def train_anal_dat_fn(self) -> dict :
        ...
        * 출력
        train_anal_dat : 데이터 프레임
        '''
        
        print('>> 학습 - 분석용 데이터 생성[종속, 설명(시차변수) 생성]')
        
        dat = self.final_summary_df.copy()
    ...
    '''
    - (예측) 종속변수/ 설명변수 지정 및 시차 변경된 설명변수 생성
    '''
    def pred_anal_dat_fn(self) -> dict :
        '''
        * 입력
        final_summary_df : PREPROCESS_FN의 출력 데이터 프레임

        * 출력
        pred_anal_dat : 데이터 프레임
        '''

        print('>> 예측 - 분석용 데이터 생성[종속, 설명(시차변수) 생성]')
        ...
        dat.rename(columns = {'암모니아_max' : 'y_암모니아', '황화수소_max' : 'y_황화수소','시간' : 'base_time'}, inplace = True)
        dat.rename(columns = {s : 'x_' + s for s in dat.columns if ('time' not in s) and ('y_' not in s)}, inplace = True)
        dat.drop(columns = ['y_암모니아','y_황화수소'], inplace = True)
        ...
        new_dat = new_dat[['base_time'] + [s for s in new_dat.columns if 'y_' in s]].copy()
        new_dat['predict_time'] = new_dat['base_time'].apply(lambda xx : pd.date_range(start = xx, periods = min(self.time_window) + 1, freq = 'H')[-1])
        ...
        #
        print("Step3. base_time 컬럼 생성")
        new_dat['base_time'] = \
        new_dat['predict_time'].apply(lambda xx : pd.date_range(end = xx, periods = min(self.time_window) + 1, freq = 'H')[0])
        
        self.pred_anal_dat = dict({'pred' : new_dat})
        return self.pred_anal_dat